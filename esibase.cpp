...
QNetworkReply* EsiBase::getMarketsRegionOrdersType(qint64 reg_id, qint64 type_id)
{
    QString path = fullUrl() + "markets/" + QString::number(reg_id) + "/orders/";
    QUrl url(path);
    QUrlQuery qurl;
    qurl.addQueryItem("type_id", QString::number(type_id));
    return getRequest(url, qurl);
}
